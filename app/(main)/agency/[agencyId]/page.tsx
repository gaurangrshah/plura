...
const Page = async ({
  ...
  // 01-01T00:00:00Z === (Jan 1 @ Midnight)
  const startDate = new Date(`${currentYear}-01-01T00:00:00Z`).getTime() / 1000
  // 12-31T23:59:59Z === (Dec 31 @ 11:59:59 PM)
  ...
  // get all sub accounts for the agency
  ...
  // get stripe account details and transactions if connected for the agency
  if (agencyDetails.connectAccountId) {
    ...
    // filter out the closed/completed transaction sessions (paid)
    ...
    // filter out the pending transaction sessions (not paid)
    ...
    potentialIncome = +totalPendingSessions // @NOTE: '+' is a hack to convert to number
      .reduce((total, session) => total + (session.amount_total || 0), 0)
      .toFixed(2)
