...
export function PipelineView({
  lanes,
  pipelineDetails,
  pipelineId, subaccountId, updateLanesOrder, updateTicketsOrder,
}: Props) {
  ...
  /**
 * Handles the drag end event for the pipeline view.
 * @param dropResult The result of the drag operation.
 */
  const onDragEnd = (dropResult: DropResult) => {
    const { destination, source, type } = dropResult
    if (
      !destination || // if no destination - item dropped outside of lane OR source == destination
    ...
    switch (type) {
      case 'lane': {
        // Reorder lanes based on dragged position and update the order in the database and UI state
      ...
      case 'ticket': {
        // Reorder tickets based on dragged position and update the order in the database and UI state
        let newLanes = [...allLanes]

        // find where the drag started
        const originLane = newLanes.find(
          (lane) => lane.id === source.droppableId
        )

        // find where the drag ended
        ...
        // if the drag started and ended in the same lane - reorder the tickets
        if (source.droppableId === destination.droppableId) {
          ...
          // if the drag started and ended in different lanes - move the ticket to the new lane
