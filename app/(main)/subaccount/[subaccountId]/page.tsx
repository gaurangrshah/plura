...
} from '@/components/global/subaccount-funnel-chart';
...
const SubaccountPageId = async ({ params, searchParams }: Props) => {
  ...
  // get current sub account details
  ...
  if (subaccountDetails.connectAccountId) {
    // get stripe account details and transactions if connected for the sub account
    ...
    // filter the data to get the total closed transaction sessions (paid)
    ...
    // filter the data to get the total pending transaction sessions (not paid)
  ...
  const funnelPerformanceMetrics = funnels.map((funnel: any) => ({
    ...
    totalFunnelVisits: funnel.FunnelPages.reduce(
      (total: number, page: any) => total + page.visits,
      0
    ),
  }))
