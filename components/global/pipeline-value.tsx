...
export const PipelineValue = ({ subaccountId }: Props) => {
  ...
  const totalPipelineValue = useMemo(() => {
    if (pipelines.length) {
      return (
        // get the total value of all tickets in the given pipeline except the last lane
        pipelines
          ...
          }, 0) || 0 // if no pipeline is selected or there is no value, return 0
      )
    }
    return 0 // if no pipelines are available, return 0
  }, [selectedPipelineId, pipelines])

  const pipelineRate = useMemo(
    () =>
      // calculate the rate of the selected pipeline
    ...
    <Card className="relative w-full xl:w-[350px]">
      ...
      <CardContent className="text-sm text-muted-foreground">
        ...
        >
          ...
          <SelectContent>
            <SelectGroup>
              <SelectLabel className="text-white/30">Pipelines</SelectLabel>
